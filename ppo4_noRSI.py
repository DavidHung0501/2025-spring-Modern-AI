...
class TradingEnv(gym.Env):
    def __init__(self, price_series, initial_balance=100000, transaction_fee_percent=0.002, tax_percent=0.003, risk_penalty_factor=0.0002):
        ...
        self.ma5, self.ma10 = compute_indicators(price_series)
...
# === train agent ===
num_agents = 5
agents = [PPOAgent(input_dim=5, output_dim=3) for _ in range(num_agents)]
agent_performances = []

for idx, agent in enumerate(agents):
    env = TradingEnv(train_prices)

    for episode in tqdm(range(150), desc=f"Agent {idx+1}/{num_agents} Training"):
        state = env.reset()
...
ma5, ma10 = compute_indicators(prices)
